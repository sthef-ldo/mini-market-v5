                        ...
                        <flux:table.row>
                            ...
                            <flux:table.cell>
                                <div class="position-relative">
                                    ...
                                    <div class="actualizar-producto d-inline-flex items-center" data-stock-id="{{ $stock->id }}">
                                        @csrf
                            ...
                            <flux:table.cell class="sub-total" data-stock-id="{{ $stock->id }}">${{ number_format($stock->pivot->sub_total, 2) }}</flux:table.cell>
        ...
        <div class="flex flex-col md:flex-row gap-8 items-center justify-between mt-8 mb-12">
            <flux:card class="flex flex-col gap-3 p-6 w-full md:w-1/2">
                <flux:text class="text-base">
                    ...
                    <span id="cart-total-items">{{ $carritos->sum(fn($carrito) => $carrito->stocks->sum('pivot.cantidad')) }}</span>
                </flux:text>

                <flux:text class="text-base">
                    <strong>Total a pagar:</strong>
                    $<span id="cart-total-price">{{ number_format($carritos->sum(fn($carrito) => $carrito->stocks->sum(fn($stock) => $stock->precio * $stock->pivot->cantidad)), 2) }}</span>
                </flux:text>
            </flux:card>
